...
public class TetrisAnimation extends JApplet implements Runnable,KeyListener{
	int x1=150;
	int y1;
	...
	public void paint(Graphics g) {
		...
		g.drawLine(360, 30, 560, 30);
		g.drawLine(360, 230, 560, 230);
		g.drawString("next:", 370, 60);
		sampleMino(g);
		if(multi>0) {
			score+=Math.pow(3, multi)*100;
		}
		g.drawString("score:"+String.valueOf(score),280,20);
		for(int i=0;i<20;i++) {
			g.drawRect(0, 30+i*30, 30, 30);
			g.drawRect(330, 30+i*30, 30, 30);
			g.drawRect(30+i*30, 600, 30, 30);
		}
		for(int i=0;i<19;i++) {
			for(int j=0;j<10;j++) {
				if(banmen[i][j]==1) {
					...
					g.drawRect((j+1)*30,(i+1)*30,30,30);
		...
		minoPattern();
		createMino(g);
		g.drawString(String.valueOf(downCount),100,100);
		if(gameOver) {
			g.setColor(Color.white);
			g.drawString("GAMEOVER", 150, 200);
			g.drawString("score", 150, 210);
			g.drawString(String.valueOf(score), 150, 220);
		}
		multi=0;
	...
	public void createMino(Graphics g) {
			...
			g.drawRect(x3, y3, 30, 30);
			g.drawRect(x4, y4, 30, 30);
	}
	...
	public void sampleMino(Graphics g) {
		...
		g.drawRect(sx1, sy1, 30, 30);
		g.drawRect(sx2, sy2, 30, 30);
		g.drawRect(sx3, sy3, 30, 30);
		g.drawRect(sx4, sy4, 30, 30);
	}
